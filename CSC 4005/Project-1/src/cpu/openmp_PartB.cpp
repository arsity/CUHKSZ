...
int main(int argc, char** argv)
{
    ...
    if (argc != 4)
    {
        std::cerr << "Invalid argument, should be: ./executable "
                     "/path/to/input/jpeg /path/to/output/jpeg {num_cores}\n";
        return -1;
    }
...
#pragma omp parallel for default(none) shared(input_jpeg, filteredImage, filter)
#pragma unroll(32)
...
#pragma unroll(FILTER_SIZE)
            ...
            filteredImage[(height * input_jpeg.width + width) *
                              input_jpeg.num_channels +
                          1] = static_cast<unsigned char>(std::round(sum_g));
            filteredImage[(height * input_jpeg.width + width) *
                              input_jpeg.num_channels +
                          2] = static_cast<unsigned char>(std::round(sum_b));
        }
    }
