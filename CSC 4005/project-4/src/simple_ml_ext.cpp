 ...
 **/
void matrix_dot(const float* A, const float* B, float* const C, size_t m, size_t n, size_t k)
{
    // BEGIN YOUR CODE
 ...
 *     C (float*): Matrix of size m * k
 **/
void matrix_dot_trans(const float* A, const float* B, float* const C, size_t n, size_t m, size_t k)
{
    // BEGIN YOUR CODE
    memset(C, 0, m * k * sizeof(float));
 ...
 **/
void matrix_minus(float* const A, const float* B, size_t m, size_t n)
{
    // BEGIN YOUR CODE
#pragma GCC unroll(8)
#pragma GCC ivdep
 ...
 *     scalar (float)
 **/
void matrix_mul_scalar(float* const C, float scalar, size_t m, size_t n)
{
    // BEGIN YOUR CODE
#pragma GCC unroll(8)
#pragma GCC ivdep
 ...
 *     scalar (float)
 **/
void matrix_div_scalar(float* const C, float scalar, size_t m, size_t n)
{
    // BEGIN YOUR CODE
 ...
 *     C (float*): Matrix of size m * n
 **/
void matrix_softmax_normalize(float* const C, size_t m, size_t n)
{
    // BEGIN YOUR CODE
#pragma GCC unroll(4)
    ...
    size_t batch)
{
    ...
    for (size_t i = 0; i < m; i += batch)
    {
        ...
        matrix_dot(X_batch, theta, h_batch, batch, n, k);
        matrix_softmax_normalize(h_batch, batch, k);
        vector_to_one_hot_matrix(y + i, Y_batch, batch, k);
        matrix_minus(h_batch, Y_batch, batch, k);
        matrix_dot_trans(X_batch, h_batch, grad_batch, batch, n, k);
        matrix_mul_scalar(grad_batch, lr / static_cast<float>(batch), n, k);
        matrix_minus(theta, grad_batch, n, k);
    }
    ...
    size_t batch)
{
    ...
    {
        // BEGIN YOUR CODE
        softmax_regression_epoch_cpp(train_data->images_matrix,
        ...
        matrix_dot(train_data->images_matrix,
            theta,
            train_result,
            train_data->images_num,
            train_data->input_dim,
            num_classes);
        matrix_dot(test_data->images_matrix,
            theta,
            test_result,
...
float mean_err(float* result, const unsigned char* labels_array, size_t images_num, size_t num_classes)
{
    ...
    vector_to_one_hot_matrix(labels_array, Y, images_num, num_classes);
    auto err = 0.0f;
