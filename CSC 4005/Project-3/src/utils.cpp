...
void checkSortResult(std::vector<int>& vec1, std::vector<int>& vec2)
{
    ...
    if (vec1.size() != vec2.size())
    {
        ...
        std::cout << "The size of the sorted vector is expected to be "
                  << vec1.size() << std::endl;
        std::cout << "But your sorted vector's size is " << vec2.size()
                  << std::endl;
        return;
    }
    for (int i = 0; i < vec1.size(); ++i)
    {
        if (vec1[i] != vec2[i])
        {
            std::cout << "Fail to pass the sorting result check!" << std::endl;
            std::cout << i
                      << "th element of the sorted vector is expected to be "
                      << vec1[i] << std::endl;
            std::cout << "But your " << i << "th element is " << vec2[i]
                      << std::endl;
            return;
        }
    }
