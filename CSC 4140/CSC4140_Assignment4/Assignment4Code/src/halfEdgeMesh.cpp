...
namespace CGL
{
    ...
    // of positions and so on).
    {
        ...
        typedef pair<Index,
                     Index> IndexPair; // ordered pair of vertex indices, corresponding to an edge of an oriented polygon
        ...
        for (PolygonListCIter p = polygons.begin(); p != polygons.end(); p++)
        {
            if (p->size() < 3)
            {
                ...
                cerr << "Error converting polygons to halfedge mesh: each polygon must have at least three vertices."
                     << endl;
                exit(1);
            }
             ...
             p++, f++)
        {
            ...
            for (Index i = 0; i < degree; i++)
            {
                ...
                {
                    cerr << "Error converting polygons to halfedge mesh: found multiple oriented edges with indices ("
                         << a << ", " << b << ")." << endl;
                    cerr
            ...
            if (h->twin() == halfedges.end())
            {
                ...
                do
                {
                    ...
                    while (i != h && // we're done if we end up back at the beginning of the loop
                           i->twin()
                           != halfedges.end()) // otherwise, we're looking for the next twinless halfedge along the loop
                    {
                        i = i->twin();
                        i = i->next();
                    }
        ...
        for (VertexIter v = vertices.begin(); v != vertices.end(); v++)
        {
            ...
            {
                cerr << "Error converting polygons to halfedge mesh: some vertices are not referenced by any polygon."
                     << endl;
                exit(1);
            }
            ...
            {
                cerr << "Error converting polygons to halfedge mesh: at least one of the vertices is nonmanifold."
                     << endl;
                exit(1);
            }
