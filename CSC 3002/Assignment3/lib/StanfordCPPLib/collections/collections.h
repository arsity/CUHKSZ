...
auto randomElement(const Collection &collection) -> const decltype(*
collection.
begin()
) & {
if (collection.
...
std::next(collection
.
begin(), randomInteger(0, collection.size() - 1)
);
}
...
#else
template<typename CollectionType, typename ElementType>
std::istream &readCollection(std::istream &input,
...
std::istream &operator>>(std::istream &is, GenericSet<SetTraits> &set) {
  ...
  return stanfordcpplib::collections::readCollection(is,
                                                     set,
                                                     element, /* descriptor */
