...
// consider 3 situations for adding an element
template<typename ValueType>
void PriorityQueue<ValueType>::enqueue(ValueType value, double priority) {
  if (count == 0) {  // add to no element quene
    ...
    count = 1;
    newCell = nullptr;
  } else if (priority < head->priority) {  // add to the first place
    Cell* newCell = new Cell;
    newCell->data = value;
    newCell->priority = priority;
    newCell->link = head;
    ...
    newCell = nullptr;
    count++;
  } else {  // add to the following of some element
    Cell* currentCell = head;
    while (currentCell->link != NULL
        && currentCell->link->priority < priority) {  // at the end or the next priority is larger
      currentCell = currentCell->link;
    }
    Cell* newCell = new Cell;
    newCell->data = value;
    newCell->priority = priority;
    newCell->link = currentCell->link;
...
ValueType PriorityQueue<ValueType>::dequeue() {
  if (count == 0) {  // check empty
    std::cerr << "There is NO element in the quene.";
    return NULL;
  } else {  // manipulate delete
    Cell* wantDelete = head;
    head = head->link;
    ValueType result = wantDelete->data;
    delete wantDelete;
    wantDelete = nullptr;
    count--;
    return result;
